...
export class WebSocketTransport implements RpcTransport {
  ...
  private constructor(socket: WebSocketSocket, options: WebSocketOptions) {
    ...
    this.#notifications = new AsyncQueue(
      options.notificationQueueSize ?? 100,
      options.notificationOverflow ?? "drop_oldest",
    );
    ...
    if (isObject(message.error)) {
      pending.reject(
        errorFromResponse(
          ...
          isObject(message.error.data) &&
            typeof message.error.data.code === "string"
            ? message.error.data.code
            : "",
          String(message.error.message ?? "Remote RPC error"),
          isObject(message.error.data) ? message.error.data.details : undefined,
        ),
      );
...
class AsyncQueue<Value> implements AsyncIterableIterator<Value> {
  ...
  readonly #overflow: "drop_oldest" | "close";

  constructor(capacity: number, overflow: "drop_oldest" | "close") {
  ...
  push(value: Value): void {
    ...
    if (this.#values.length === this.#capacity) {
      ...
      if (!this.#overflowWarned) {
        console.warn(
          "The WebSocket notification queue overflowed; dropping oldest notifications",
        );
