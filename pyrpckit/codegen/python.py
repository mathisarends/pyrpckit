...
def render_files(ir: ClientIr, options: PythonClientOptions) -> dict[str, str]:
    ...
    if ir.namespaces:
        ...
        files.update(
            {
                f"namespaces/{namespace.name}.py": render_namespace(
                    ir, namespace, options
                )
                for namespace in ir.namespaces
            }
        )
...
def _module(
    ...
    *,
    ...
    header = (
        f"# Generated by pyrpckit from {options.source}.\n"
...
_STANDARD_LIBRARY = frozenset(
    {"collections", "datetime", "enum", "typing", "uuid", "__future__"}
)
